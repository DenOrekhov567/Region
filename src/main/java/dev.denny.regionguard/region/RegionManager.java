...
import dev.denny.regionguard.utils.DatabaseManager;
import dev.denny.regionguard.utils.ResponseRegion;
import dev.denny.regionguard.utils.ResponseRegionMembers;
...
public class RegionManager {
    ...
    public ResponseRegion getRegion(Position position) {
    ...
    public boolean isCanCreate(Gamer player) {
        return player.getAccount().getData().getCountPrivates() < 3;
    }
    ...
    public void createRegion(Gamer player, String name, Block block) {
        ...
        player.sendMessage(requestEnd);
        getDatabaseManager().queryEmpty(requestEnd, false);

        player.getAccount().getData().countPrivates += 1;
        player.getAccount().update("countPrivates");
    }

    //Метод для удаления региона
    public void deleteRegion(Gamer player, String regionName) {
        getDatabaseManager().queryEmpty(String.format("DELETE FROM regions WHERE name = '%1$s'", regionName), false);
        player.getAccount().getData().countPrivates += 1;
        player.getAccount().update("countPrivates");
    }

    public Boolean isMember(Gamer player, String regionName) {
        String name = player.getName().toLowerCase();
        List<ResponseRegionMembers> list = getDatabaseManager().queryMembers(String.format("SELECT * FROM regions_members WHERE regionName = '%1$s' AND name = '$2$s';", regionName, name));
        for(ResponseRegionMembers member : list) {
